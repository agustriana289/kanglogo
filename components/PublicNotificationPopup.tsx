...
export default function PublicNotificationPopup() {
      ...
      if (storeOrders) {
        storeOrders.forEach((order: any) => {
          allPurchases.push({
            ...
            customerName: order.customer_name,
            productName:
              order.marketplace_assets?.nama_aset || "Produk Digital",
  ...
  return (
    <div
      className={`hidden md:fixed z-50 bottom-10 left-16 transition-all duration-500 ease-in-out ${
        isVisible
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-gray-800 leading-snug">
              <span className="font-semibold">
                {anonymizeName(currentPurchase.customerName)}
              </span>
              {" baru saja membeli"}
            </p>
