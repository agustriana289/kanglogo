...
export default function CategoryArticles({ categoryName }: CategoryArticlesProps) {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow">
            <h3 className={`text-xl font-semibold mb-4 ${getCategoryColor(categoryName)} border-b pb-2`}>
                {categoryName}
            </h3>
            <div className="space-y-3">
                {articles.map((article) => (
                    <div key={article.id} className="article">
                        <Link href={getArticleUrl(article)} className="block">
                            ...
                            <p className="text-sm text-gray-500 mt-1">{formatDate(article.published_at)}</p>
                        </Link>
                    </div>
                ))}
                {articles.length === 0 && (
                    <p className="text-gray-500 text-sm">Belum ada artikel di kategori ini</p>
                )}
            </div>
        </div>
    );
