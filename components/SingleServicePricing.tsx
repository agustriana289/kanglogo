      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-6 place-items-center">
          {service.packages.map((plan, index) => (
            <div
              key={index}
              className={`flex-col p-6 bg-white rounded-2xl shadow-md transform transition-transform items-center ${plan.borderColor} border`}
            >
              <div className="mb-6">
                <div
                  className={`text-center pb-4 mb-6 border-b ${plan.borderColor}`}
                >
                ...
                <div className="px-6">
                  <ul className="space-y-2">
                    {plan.features.map((feature, i) => (
                      <li
                        key={i}
                        className={`flex items-center ${plan.bodyColor}`}
                      >
                        <div className="mr-2">
                          <svg
              ...
              <div className="text-center">
                ...
                <a
                  ...
                  href={`/order/new?service=${service.slug
                    }&package=${encodeURIComponent(plan.name)}`}
                >
                  <LogoPathAnimation className="mr-2 w-4 md:w-6 aspect-square" />
                  {plan.duration}
                </a>
                <p
