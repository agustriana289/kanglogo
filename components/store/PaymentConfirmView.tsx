...
export default function PaymentConfirmView({ invoiceNumber }: PaymentConfirmViewProps) {
    ...
    useEffect(() => {
        ...
        `)
            ...
            if (error || !orderData || orderData.status !== "pending_payment") {
                router.push(`/store/${invoiceNumber}`);
                return;
            }
            setOrder(orderData);
    ...
    return (
        <div className="min-h-screen bg-slate-100 py-12 px-4 font-sans">
            <div className="max-w-xl mx-auto">
                <button
                    onClick={() => router.push(`/store/${invoiceNumber}`)}
                    ...
                    {!success ? (
                        <>
                            <div className="bg-primary px-10 py-10 text-white relative">
                                <div className="relative z-10">
                                    ...
                                    <p className="text-white/70 text-sm font-medium">Unggah bukti transfer untuk pesanan produk digital Anda</p>
                                </div>
                                ...
                                <div className="bg-slate-50 rounded-2xl p-7 space-y-4 border border-slate-200">
                                    <div className="space-y-3 text-sm">
                                        <div className="flex justify-between items-center group">
                                            <span className="text-slate-500 flex items-center gap-2 font-medium"><FileText size={14} /> Nomor Pesanan</span>
                                            <span className="text-slate-800 font-bold font-mono bg-white px-2 py-0.5 rounded border border-slate-200 group-hover:border-primary/30 transition-colors">#{order?.order_number}</span>
                                        </div>

                                        <div className="flex justify-between items-center">
                                            <span className="text-slate-500 flex items-center gap-2 font-medium"><ShoppingBag size={14} /> Nama Produk</span>
                                            <span className="text-slate-800 font-bold text-right max-w-[200px] break-words">{order?.marketplace_assets?.nama_aset}</span>
                                        </div>

                                        <div className="flex justify-between items-center">
                                            <span className="text-slate-500 flex items-center gap-2 font-medium"><User size={14} /> Nama Pembeli</span>
                                        ...
                                        <div className="pt-4 border-t border-slate-200 flex justify-between items-end mt-2">
                                            ...
                                            <span className="text-3xl font-black text-primary leading-none">Rp {order?.price.toLocaleString("id-ID")}</span>
                                        </div>
                                    </div>

                                    {paymentMethod && (
                                        <div className="mt-6 pt-5 border-t border-slate-200">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Rekening Tujuan</p>
                        ...
                        <div className="p-16 text-center space-y-8 animate-in fade-in zoom-in duration-500">
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-slate-500 leading-relaxed max-w-[340px] mx-auto">Terima kasih! Bukti pembayaran Anda telah kami terima dan akan segera diverifikasi agar file dapat Anda unduh.</p>
                            </div>
