...
export default function Featured() {
  ...
  if (loading) {
    return (
      <section className="pt-12 px-4 py-8 sm:px-6 sm:py-12 lg:px-8 relative" id="featured">
  ...
  return (
    <section className="pt-12 px-4 py-8 sm:px-6 sm:py-12 lg:px-8 relative" id="featured">
      ...
      <div className="divider absolute top-0 left-0 w-full">
        <svg className="transform rotate-180 absolute bottom-0 fill-slate-100 h-[150px] w-full" preserveAspectRatio="none" viewBox="0 0 1000 37" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <div className="text-center mb-12">
          <h1 className="max-w-2xl mx-auto font-manrope font-bold text-4xl text-slate-700 mb-5 md:text-6xl leading-tight">
            ...
            {content.featured_subtitle && <span className="text-primary ml-2">{content.featured_subtitle}</span>}
          </h1>
            ...
            <div key={num} className="bg-white rounded-3xl p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-center">
                {content[`featured_icon${num}`] && (
                  <div className="size-16 text-primary" dangerouslySetInnerHTML={{ __html: content[`featured_icon${num}`] }} />
                )}
              </div>
              <div className="text-center">
                <h2 className="text-2xl font-bold text-primary mb-4">{content[`featured_title${num}`]}</h2>
                <p className="text-base font-normal leading-7 text-slate-700">{content[`featured_desc${num}`]}</p>
              </div>
            </div>
          ))}
        </div>
