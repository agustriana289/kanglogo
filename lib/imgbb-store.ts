// lib/imgbb-store.ts
export async function uploadImageToImgBBStore(
  imageFile: File
): Promise<string> {
  ...
  if (!apiKey) {
    throw new Error(
      `API key ImgBB untuk marketplace tidak ditemukan. Pastikan NEXT_PUBLIC_IMGBB_API_KEY3 sudah diatur di file .env`
    );
  }
