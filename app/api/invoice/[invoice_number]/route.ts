...
export async function GET(request: NextRequest, { params }: { params: { invoice_number: string } }) {
    ...
    try {
      ...
      `)
        ...
        doc.text(`Tanggal: ${new Date(order.created_at).toLocaleDateString('id-ID')}`, 105, 37, { align: 'center' });
        ...
        doc.setFontSize(12);
        doc.setFont(undefined, 'bold');
        doc.text(`Total: Rp ${order.final_price.toLocaleString('id-ID')}`, 14, finalY + 20);
