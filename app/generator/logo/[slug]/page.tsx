...
export default function LogoGeneratorDetailPage() {
    ...
    const [sloganFontFamily, setSloganFontFamily] = useState("'Inter', sans-serif");
    const [selectedSloganFont, setSelectedSloganFont] = useState<LogoFont | null>(null);
    const [sloganBaseFontSize, setSloganBaseFontSize] = useState(100);

    // Logo Icon Color State
    const [logoColorMode, setLogoColorMode] = useState<"original" | "solid">("original");
    ...
    const fetchAssetAndFonts = async () => {
        try {
            ...
            if (fontError) {
                ...
                // Fallback
                const fallback = { id: 0, font_name: 'Inter', google_font_family: 'Inter:wght@400;700', created_at: '' };
    ...
    const handleFontChange = (e: React.ChangeEvent<HTMLSelectElement>, type: "main" | "slogan") => {
    ...
    useEffect(() => {
        ...
        asset, companyName, sloganText, showSlogan, layout,
        baseFontSize, fontFamily, // Main Text Dependencies
        sloganBaseFontSize, sloganFontFamily // Slogan Dependencies
    ]);
    ...
    const calculateLayout = () => {
        ...
        ctx.font = `bold ${mainFontSize}px ${fontFamily.split(',')[0].replace(/'/g, "")}`;
        ...
        if (showSlogan) {
            ctx.font = `normal ${sloganFontSize}px ${sloganFontFamily.split(',')[0].replace(/'/g, "")}`;
        ...
        const totalTextHeight = mainTextHeight + (showSlogan ? sloganHeight + 20 : 0);
        ...
        const padding = 50;
        setViewBox(`${-padding} ${-padding} ${totalWidth + padding * 2} ${totalHeight + padding * 2}`);
    };
    ...
    const mainFontStr = selectedFont ? selectedFont.google_font_family : 'Inter:wght@400;700';
    const sloganFontStr = selectedSloganFont ? selectedSloganFont.google_font_family : 'Inter:wght@400;700';
    ...
    const importUrls = [`https://fonts.googleapis.com/css2?family=${mainFontStr}&display=swap`];
    if (showSlogan && mainFontStr !== sloganFontStr) {
        importUrls.push(`https://fonts.googleapis.com/css2?family=${sloganFontStr}&display=swap`);
    }
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
                            <div className="bg-slate-50/50 p-4 border-b border-slate-100 flex justify-between items-center">
                                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Preview Canvas</span>
                            ...
                            <div className="p-8 lg:p-12 flex items-center justify-center bg-[url('https://transparenttextures.com/patterns/cubes.png')] bg-slate-50 min-h-[500px]">
                                ...
                                >
                                    <defs>
                                        <style>
                                            {`
                                    ${importUrls.map(url => `@import url('${url}');`).join('\n')}
                                ...
                                `}
                                    ...
                                    <g transform={`translate(${layoutMetrics.iconX}, 0)`} className="icon-wrapper">
                                        <svg
                                            width="400"
                                            height="400"
                                            viewBox="0 0 1080 1080"
                                            dangerouslySetInnerHTML={{ __html: asset.svg_content.replace(/<svg[^>]*>|<\/svg>/g, '') }}
                                        />
                                    </g>

                                    <g transform={`translate(${layoutMetrics.textX}, 0)`} textAnchor={layout === 'right' ? 'end' : 'start'}>
                                        <text
                    ...
                    <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 space-y-8 sticky top-6">
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2 border-b pb-2 border-slate-100">
                                <DocumentTextIcon className="w-4 h-4 text-primary" /> Konten Text
                            </h3>
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between">
                                    <label className="flex items-center gap-2 cursor-pointer select-none">
                                        ...
                                        />
                                        <span className="text-sm font-medium text-slate-700">Tambah Slogan</span>
                                    </label>
                                </div>

                                {showSlogan && (
                                    <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                                        <label className="block text-xs font-semibold text-slate-600 mb-1.5">Isi Slogan</label>
                                        <input
                                            type="text"
                                            value={sloganText}
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2 border-b pb-2 border-slate-100">
                                <AdjustmentsHorizontalIcon className="w-4 h-4 text-primary" /> Tipografi
                            </h3>
                            <div className="space-y-4">
                                {/* MAIN TEXT FONT */}
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Font Utama</label>
                                    <select
                                    ...
                                    >
                                        {availableFonts.length === 0 && <option>Loading fonts...</option>}
                                ...
                                <div className="pb-4 border-b border-slate-100">
                                    <div className="flex justify-between items-center mb-1.5">
                                        <label className="block text-xs font-semibold text-slate-600">Ukuran Text Utama</label>
                                        <span className="text-xs text-slate-400 font-mono">{baseFontSize}%</span>
                                    </div>
                                    <input
                                        type="range"
                                        min="50"
                                ...
                                {showSlogan && (
                                    <div className="animate-in fade-in space-y-4">
                                        <div>
                                            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Font Slogan</label>
                                            <select
                                        ...
                                        <div>
                                            <div className="flex justify-between items-center mb-1.5">
                                                <label className="block text-xs font-semibold text-slate-600">Ukuran Slogan</label>
                                                <span className="text-xs text-slate-400 font-mono">{sloganBaseFontSize}%</span>
                                            </div>
                                            <input
                                                type="range"
                                                min="50"
                                                ...
                                                onChange={(e) => setSloganBaseFontSize(Number(e.target.value))}
                                                className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-primary"
                                            />
                                        </div>
                                    </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between p-2 rounded-lg border border-slate-100 hover:border-slate-200 transition">
                                    <label className="text-sm font-medium text-slate-700">Warna Utama</label>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-slate-400 font-mono uppercase bg-slate-50 px-1.5 py-0.5 rounded">{companyColor}</span>
                                        <input
                                            type="color"
                                            value={companyColor}
                                ...
                                {showSlogan && (
                                    <div className="flex items-center justify-between p-2 rounded-lg border border-slate-100 hover:border-slate-200 transition animate-in fade-in">
                                        <label className="text-sm font-medium text-slate-700">Warna Slogan</label>
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-400 font-mono uppercase bg-slate-50 px-1.5 py-0.5 rounded">{sloganColor}</span>
                                            <input
                                                type="color"
                                                value={sloganColor}
                                ...
                                <div className="pt-2">
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="text-xs font-semibold text-slate-600">Warna Logo Icon</label>
                                        {logoColorMode === 'solid' && (
                                            <button
                                    ...
                                    <div className="grid grid-cols-2 gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
                                        <button
                                            ...
                                            className={`px-3 py-2 rounded text-xs font-medium transition-all ${logoColorMode === 'original' ? 'bg-white shadow text-primary ring-1 ring-primary/20' : 'text-slate-500 hover:bg-white/50'}`}
                                        >
                                            Warna Asli
                                        </button>
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className={`flex-1 px-3 py-2 rounded text-xs font-medium transition-all text-center ${logoColorMode === 'solid' ? 'bg-white shadow text-primary ring-1 ring-primary/20' : 'text-slate-500 hover:bg-white/50'}`}
                                            >
                                                Solid Color
                                            </button>
                                            {logoColorMode === 'solid' && (
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2 border-b pb-2 border-slate-100">
                                <ArrowsRightLeftIcon className="w-4 h-4 text-primary" /> Tata Letak
                            </h3>
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    onClick={() => setLayout("left")}
                                    className={`p-3 rounded-lg border-2 flex flex-col items-center gap-2 transition-all ${layout === 'left' ? 'border-primary bg-primary/5 text-primary scale-[1.02] shadow-sm' : 'border-slate-100 hover:border-slate-200 text-slate-500'}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`p-3 rounded-lg border-2 flex flex-col items-center gap-2 transition-all ${layout === 'right' ? 'border-primary bg-primary/5 text-primary scale-[1.02] shadow-sm' : 'border-slate-100 hover:border-slate-200 text-slate-500'}`}
                                >
                                    <div className="flex items-center gap-1 font-mono text-[10px] border border-current p-1.5 rounded bg-white">
                                        <div className="h-1 w-6 bg-current rounded-full opacity-30"></div>
                                        <div className="w-3 h-3 bg-current rounded-full opacity-50"></div>
