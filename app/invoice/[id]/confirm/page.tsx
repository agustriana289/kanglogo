  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (settings?.website_phone) {
        setWaNumber(
          settings.website_phone.replace(/[^0-9]/g, "").startsWith("0")
          ...
          {!success ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-3">
                    <label className="text-sm font-bold text-slate-700 ml-1">
                      Unggah Screenshot Bukti
                    </label>
                    <label
                      className={`relative flex flex-col items-center justify-center border-2 border-dashed rounded-2xl p-10 transition-all cursor-pointer group ${
                        proofFile
                        ...
                        <div className="text-center w-full">
                          {filePreview ? (
                            <img
                              src={filePreview}
                              alt="Preview"
                              className="max-h-56 rounded-xl border border-slate-200 mx-auto mb-4 shadow-sm"
                            />
                          ) : (
                          ...
                          <p className="text-primary font-bold break-all px-4">
                            {proofFile.name}
                          </p>
                          <p className="text-slate-400 text-xs mt-1">
                            Klik untuk mengganti screenshot
                          </p>
                        </div>
                      )}
                    </label>
            ...
            <div className="p-16 text-center space-y-8 animate-in fade-in zoom-in duration-500">
              ...
              <div className="pt-4 space-y-4">
                ...
                >
                  <MessageCircle size={26} fill="white" /> Konfirmasi via
                  WhatsApp
                </a>
