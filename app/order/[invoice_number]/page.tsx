          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-primary/5 overflow-hidden">
            ...
            <div className="bg-primary px-8 py-10 text-white relative">
              <div className="relative z-10">
                ...
                <p className="text-white/80">Konfirmasi detail pesanan dan status pembayaran Anda</p>
              </div>
              <div className="absolute top-0 right-0 p-8 flex flex-col items-end gap-2 opacity-20 sm:opacity-100">
                <span className="text-xs uppercase tracking-widest font-medium text-white/60">Nomor Invoice</span>
                <span className="text-2xl font-mono font-bold">#{order.invoice_number}</span>
              </div>
            </div>
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="flex flex-wrap gap-3 pb-8 border-b border-slate-100">
                ...
                >
                  <Download size={18} className="text-primary" /> {getDownloadButtonText()}
                </button>
                <button
                ...
                >
                  <Download size={18} className="rotate-180 text-primary" /> Cetak
                </button>
                <button
                  onClick={copyLink}
                ...
                >
                  <Copy size={18} className="text-primary" /> {copied ? "Berhasil Disalin!" : "Salin Tautan"}
                </button>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Detail Pembayaran</h3>
                  <div className="bg-slate-50 p-6 rounded-2xl space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-slate-800 text-sm">{paymentMethod?.name || "Belum ditentukan"}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-500 text-sm">Tanggal</span>
                      <span className="font-semibold text-slate-800 text-sm">{formatDate(order.created_at)}</span>
                    </div>
                  </div>
                </div>

                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Informasi Pelanggan</h3>
                  <div className="bg-slate-50 p-6 rounded-2xl space-y-3 text-sm">
                    ...
                    {order.customer_whatsapp && (
                      <div>
                        ...
                        <p className="font-medium text-slate-700">{order.customer_whatsapp}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Order Items Table */}
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Layanan yang Dipesan</h3>
                  ...
                  <div className="block sm:hidden p-6 space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">Estimasi: {order.package_details?.duration || "-"}</p>
                      </div>
                    </div>
                    <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                      <span className="text-slate-500 text-sm">Harga Paket</span>
                      <span className="font-bold text-slate-800">Rp {priceBeforeDiscount.toLocaleString("id-ID")}</span>
                    </div>
                  </div>
                  ...
                  <table className="hidden sm:table w-full text-left">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase text-center">Estimasi</th>
                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase text-right">Harga</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      <tr>
                        <td className="px-6 py-5">
                          <p className="font-bold text-slate-800">{order.package_details?.name || "Paket Layanan"}</p>
                          <p className="text-xs text-slate-400 mt-1">Layanan Desain KangLogo</p>
                        </td>
              ...
              <div className="grid md:grid-cols-2 gap-8 pt-4">
                  ...
                  {order.status === "pending_payment" && order.payment_deadline && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-6">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          <p className="font-bold text-yellow-800">Batas Pembayaran</p>
                          <p className="text-xs text-yellow-700/70">Segera selesaikan sebelum waktu habis</p>
                        </div>
                      </div>
                      <div className="flex gap-2 justify-center">
                        {[
                          ...
                          <div key={idx} className="flex flex-col items-center">
                            ...
                            <span className="text-[10px] font-bold text-yellow-700/50 mt-1">{t.label}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {paymentMethod && (
                    <div className="mt-6 p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h4 className="font-bold text-slate-800">Instruksi Pembayaran</h4>
                      </div>
                      <div className="space-y-3 text-sm">
                        <div className="flex justify-between"><span className="text-slate-500">Bank/E-Wallet</span><span className="font-bold text-slate-800">{paymentMethod.name}</span></div>
                        <div className="flex justify-between items-center"><span className="text-slate-500">Nomor Rekening</span><span className="font-mono font-bold text-primary bg-primary/5 px-2 py-0.5 rounded tracking-wider">{paymentMethod.account_number}</span></div>
                        <div className="flex justify-between"><span className="text-slate-500">Atas Nama</span><span className="font-bold text-slate-800">{paymentMethod.holder_name}</span></div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 p-6 rounded-2xl space-y-3">
                    ...
                    {discountAmount > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Diskon {order.discount_code ? `(${order.discount_code})` : ""}</span>
                        <span className="font-bold text-green-600">- Rp {discountAmount.toLocaleString("id-ID")}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="pt-3 border-t border-slate-200">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-2xl font-black text-primary">Rp {order.final_price.toLocaleString("id-ID")}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {order.status === "pending_payment" ? (
                    <button
                      onClick={() => router.push(`/order/${order.invoice_number}/confirm`)}
            ...
            <div className="bg-slate-50 px-8 py-6 text-center border-t border-slate-100">
              <p className="text-sm text-slate-400">
                Pertanyaan? Hubungi tim support kami di <span className="text-primary font-bold">halo@kanglogo.com</span>
              </p>
            </div>
          </div>
        </div>
