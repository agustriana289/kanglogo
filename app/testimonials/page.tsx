...
export default function TestimonialsPage() {
  ...
  const [filterSource, setFilterSource] = useState<"all" | "service" | "store">(
    "all"
  );
  ...
  useEffect(() => {
    ...
    if (filterRating) {
      filtered = filtered.filter(
        (t) => Math.round(getAverageRating(t)) === filterRating
      );
    }
    ...
    <main className="min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Menampilkan {currentItems.length} dari {filteredTestimonials.length}{" "}
          testimoni
        </p>
