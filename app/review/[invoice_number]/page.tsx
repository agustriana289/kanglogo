...
export default function SubmitTestimonialPage() {
  ...
  const [orderType, setOrderType] = useState<"service" | "store" | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Cek di orders terlebih dahulu (service)
        ...
        if (orderData && !orderError) {
          ...
          console.log("Services:", orderData.services);
          console.log("Package Details:", orderData.package_details);
        ...
        // Jika tidak ada di orders, cek store_orders
        const { data: storeData, error: storeError } = await supabase
          .from("store_orders")
          .select(
            "id, order_number, customer_name, customer_email, marketplace_assets(nama_aset)"
          )
          .eq("order_number", invoice_number)
          .maybeSingle();

        if (storeData && !storeError) {
          setOrderInfo(storeData);
          setOrderType("store");

          // Cek apakah sudah ada testimoni untuk store order ini
          const { data: testiData } = await supabase
            .from("testimonials")
            .select("*")
            .eq("store_order_id", storeData.id)
            .maybeSingle();
    ...
    try {
      if (testimonial) {
        ...
        if (orderType === "service") {
          updateData.service_name = orderInfo?.services?.title || "Layanan KangLogo";
          updateData.package_name = orderInfo?.package_details?.name || null;
          updateData.product_name = null;
        } else if (orderType === "store") {
          updateData.service_name = null;
          updateData.package_name = null;
          updateData.product_name = orderInfo?.marketplace_assets?.nama_aset || "Produk Digital";
        }
        ...
        if (orderType === "service") {
          ...
          insertData.store_order_id = null;
          insertData.service_name = orderInfo?.services?.title || "Layanan KangLogo";
          insertData.package_name = orderInfo?.package_details?.name || null;
          insertData.product_name = null;
        } else if (orderType === "store") {
          insertData.order_id = null;
          insertData.store_order_id = orderInfo.id;
          insertData.service_name = null;
          insertData.package_name = null;
          insertData.product_name = orderInfo?.marketplace_assets?.nama_aset || "Produk Digital";
        }
                ...
                <p className="font-semibold text-gray-800">
                  {orderType === "service"
                    ? orderInfo?.services?.title && orderInfo?.package_details?.name
                      ? `${orderInfo.services.title} (${orderInfo.package_details.name})`
                      : orderInfo?.services?.title || "Layanan KangLogo"
                    : orderInfo?.marketplace_assets?.nama_aset ||
