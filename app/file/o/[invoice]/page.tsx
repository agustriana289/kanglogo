...
export default function OrderFileManagerPage({
    params,
}: {
    ...
    const fetchOrderAndFiles = async () => {
        ...
        try {
            ...
            const { data: order, error: orderError } = await supabase
                .from("orders")
                .select("invoice_number, customer_name, customer_email, final_file_link, status")
            ...
            await fetchFolderContents(order.final_file_link);
    ...
    const handleOpenInDrive = () => {
        if (orderData?.final_file_link) {
            window.open(orderData.final_file_link, "_blank");
        }
    };
    ...
    return (
        <InvoiceGate customerEmail={orderData.customer_email} invoiceNumber={orderData.invoice_number}>
            <div className="min-h-screen bg-slate-100 dark:bg-slate-900 py-8">
                <div className="container mx-auto px-4 max-w-6xl">
                    {/* Header */}
                    <div className="mb-6">
                        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">
                            File Akhir untuk Proyek {orderData.invoice_number}
                        </h1>
                    </div>
